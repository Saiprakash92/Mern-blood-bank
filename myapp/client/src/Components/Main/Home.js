...
const Home = () => {
    ...
    return (
     ...
     <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'row', animation: 'moveQuotes 10s infinite' }}>
  <img src={bg} alt="" style={{ flex: '75%', maxWidth: '75%' }} />
  <div style={{ flex: '25%', padding: '0 20px' }}>
    {/* Add two quotes with larger font */}
    <p style={{ margin: 0, fontSize: '2rem', fontWeight: 'bold', fontFamily: 'Arial', color: '#333' }}>
      "Donate blood, be a hero, and save lives!"
    </p>
    <p style={{ margin: 0, fontSize: '2rem', fontWeight: 'bold', fontFamily: 'Helvetica', color: '#990000' }}>
      "Your blood donation can bring hope and happiness to many lives."
    </p>
  </div>
</div>
        ...
        <p className='text-justify'>
          ...
          {i === 3 && "Witness the Impact: Experience the power of your donation through inspiring stories shared by recipients, their families, and donors. Discover how your contribution through Raktdhara has made a difference in people's lives and fostered a sense of community and hope."}
            ...
            <div className='w-full bg-blood text-white-900 h-max text-sm text-center font-bold'>
                <code>|Hare krishna| BloodLink @ {new Date().getFullYear()} || Made with ❤️ by Prakash singh</code>
            </div>
        </div>
    )
}
